 ...
 */
function danytheme_theme(&$existing, $type, $theme, $path) {
...
function danytheme_preprocess_page(&$vars, $hook) {
  ...
  $vars['custom_colors']= '';
  $color1 = theme_get_setting('danytheme_color1');
  $color2 = theme_get_setting('danytheme_color2');
  $color3 = theme_get_setting('danytheme_color3');
  $color4 = theme_get_setting('danytheme_color4');
  $color5 = theme_get_setting('danytheme_color5');
  $color6 = theme_get_setting('danytheme_color6');
  $color7 = theme_get_setting('danytheme_color7');
  $color8 = theme_get_setting('danytheme_color8');
...
function danytheme_node_submitted($node) {
    if ($node->uid == 1) {
        // Daniel 20-Jun-2010: nodes authored by User 1 have simpler date-format
        $tmp1 = getdate($node->created);
        $nodeyear = $tmp1['year'];
        $tmp1 = getdate();
        $curryear = $tmp1['year'];
        $dformat =  ($nodeyear == $curryear ? 'D j F, g:i a' : 'Y - j F' );
        return t('@datetime',
            array('@datetime' => format_date($node->created,'custom',$dformat)
        ));
    }
  // Default behavior - copied from modules/node/node.module theme_node_submitted()
  return t('From !username on @datetime',
    array(
